...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:ᬊ͜͡𝑇𝐵¹⁵⁷-ZEUS↯\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5514998435607:+5514998435607\n' //Nomor whatsapp kamu
            + 'END:VCARD'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD ๛𝙳𝙸𝙴𝙶𝙸𝙽𝟸𝙺𖨐 BOT*',
					...
					admin: '[❗] Este comando só pode ser usado por administradores de grupo e o dono! ❌',
					Badmin: ' [❗] Este comando só pode ser usado quando o bot se torna administrador! ❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5514998435607@s.whatsapp.net","12267740582@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5514998435607@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5514998435607@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5514998435607@s.whatsapp.net"]//ubah nomor lo
			const  premium= ["5514998435607@s.whatsapp.net","5516994100461@s.whatsapp.net","5517991896064@s.whatsapp.net","5514998435607@s.whatsapp.net"]
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
		...
		setTimeout( () => {
	        ...
	        if (messagesC.includes("macaco","seu preto")){
		if (!isGroup) return
		if (!isAntiRacismo) return
		if (isGroupAdmins) return reply('cara, nao fale essas coisas, é errado, mas vc e admin n irei te banir')
		client.updatePresence(from, Presence.composing)
...
const get = require('got')
	...
	if (messagesC.includes("sua mae","tua mae")){
		...
		if (messagesC.includes("kkkkkk")){
			...
			client.updatePresence(from, Presence.composing)
			reply("to bem, vlw por perguntar abiginho 😏")
	}
	...
	if (messagesC.includes("tag1")){
			client.updatePresence(from, Presence.composing)
			reply("⏤͟͞ঔৣ͜͡NICK⟅  ⃝⃕🇹🇷")
			
	}
	
	if (messagesC.includes("tag2")){
			client.updatePresence(from, Presence.composing)
			reply("𝐓𝐐𝐒 NOME🇹🇷")
		...
		if (messagesC.includes("vsfd")){
			client.updatePresence(from, Presence.composing)
			reply("bora juntos? vai ser melhor nois dois?")
	}
		...
		if (messagesC.includes("bot")){
			client.updatePresence(from, Presence.composing)
			reply("oi meu lindo(a)")
	}
	
	    if (messagesC.includes("Quer namora comigo?")){
			client.updatePresence(from, Presence.composing)
			reply("clarooo❤. oi amor")
        ...
        case 'promover':
				...
				case 'bemvindo':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Desativou com sucesso o recurso de boas-vindas neste grupo, uffa tava chato')
					} else {
